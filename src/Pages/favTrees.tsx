...
const FavTreesMenu: React.FC<PageProps> = (props) => {
  ...
  const [isTreeLimitReached, setIsTreeLimitReached] = useState(false); // number of max fav trees allowed

  // listen to changes in firestore database
  useEffect(() => {
    firestore
      .collection(`users/${props.userID}/favTrees`)
      .onSnapshot((snapshot) => {
  ...
  // watches existingTrees and updates isTreeLimitReached accordingly
  useEffect(() => {
    if (existingTrees.length === 3) {
      setIsTreeLimitReached(true);
    } else {
      setIsTreeLimitReached(false);
    }
  }, [existingTrees]);
  ...
  useEffect(() => {
    ...
    const grabImgURL = async (favTreeId: number) => {
      ...
      const imgRef = storage.ref(
        `${props.userID}/favTrees/favTree${favTreeId}`
      );
  ...
  return (
    <>
      ...
      <AddTreeForm
        ...
        isTreeLimitReached={isTreeLimitReached}
            ...
            (existingTree: firebase.firestore.DocumentData) => {
              return (
                <li key={existingTree.favTreeId}>
                  <Link to={`${url}/favTree${existingTree.favTreeId}`}>
                    <FavTreeCard
